...
function Doctorr() {
  ...
  const handleAddDoctor = (newDoctor) => {
    ...
    axios.post(' http://51.20.5.215:5000/doctors', newDoctor)
      .then((response) => {
  ...
  useEffect(() => {
    axios.get(' http://51.20.5.215:5000/Doctorr')
      .then((response) => {
  ...
  const handleDeleteDoctor = (doctorId) => {
    ...
    axios.delete(`http://51.20.5.215:5000/doctors/${doctorId}`)
      .then(() => {
  ...
  const handleEditDoctor = (editedDoctor) => {
    ...
    axios.put(`http://51.20.5.215:5000/${editedDoctor.id}`, editedDoctor)
      .then(() => {
