...
function Patient() {
  ...
  const handleAddPatient = (newPatient) => {
    ...
    axios.post(' http://51.20.5.215:5000/patients', newPatient)
      .then((response) => {
  ...
  useEffect(() => {
    axios.get('http://51.20.5.215:5000/patients')
  .then((response) => {
  ...
  const handleDeletePatient = (PatientId) => {
    ...
    axios.delete(`http://51.20.5.215:5000/Patients/${PatientId}`)
      .then(() => {
  ...
  const handleEditPatient = (editedPatient) => {
    ...
    axios.put(` http://51.20.5.215:5000/Patients/${editedPatient.id}`, editedPatient)
      .then(() => {
